...
Explorer uses copies in `%LOCALAPPDATA%\FeatherPad\FileIcons`, named by content hash to avoid reusing old EXE resource cache entries after upgrades. Embedded icons remain available for the portable executable. Registration updates FeatherPad's ProgIDs and extension-specific auto ProgIDs that point to this exact executable.
